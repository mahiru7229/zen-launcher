...
class ModpackLifecycleController(BaseController):
    ...
    def scan(self, instance_name: str) -> None:
        ...
        self._task_runner.run("modpack.scan", lambda: ModrinthPackRegistry.scan(InstanceManager.load(name), reporter=reporter), tr("Scanning Modrinth pack files for '{name}'...", name=name), blocking=False)
    ...
    def repair(self, instance_name: str) -> None:
        ...
        if not name:
            return
        reporter = ProgressReporter(self.progress_received.emit)
        self._task_runner.run("modpack.repair", lambda: ModrinthPackRepairManager.repair(InstanceManager.load(name), reporter=reporter), tr("Repairing Modrinth modpack for '{name}'...", name=name))
    ...
    def _on_task_succeeded(self, task_id: str, result: object) -> None:
        ...
        if task_id == "modpack.update.apply":
            ...
            return
        if task_id == "modpack.repair":
            self.repair_finished.emit(result)
            self.status_changed.emit(tr("Modrinth modpack repaired"))
            self.log_created.emit(tr("Modrinth modpack repaired: {count} file(s) restored", count=getattr(result, "repaired_files", 0)))
