...
Tài liệu này liệt kê toàn bộ PNG mà một theme có thể thay thế. Mọi file đều là **tùy chọn**: nếu thiếu, hỏng, không phải PNG hợp lệ hoặc nằm ngoài thư mục theme, launcher tự dùng CSS/text mặc định và tiếp tục chạy.
...
Canvas dưới đây là kích thước thiết kế khuyến nghị. Launcher có thể co giãn ảnh; nên giữ viền pixel quan trọng cách mép ít nhất 8–18 px để dùng tốt với nine-slice/stretch.
...
- Không đổi tên key trong `theme.json`.
...
```json
{
  ...
  "assets": {
    ...
    "button.primary": "controls/buttons/primary.png",
    "icon.action.launch": "icons/actions/launch.png"
  }
}
```

Không cần khai báo những asset chưa có. Manifest `themes/mcw-default/theme.json` trong repo đã khai báo sẵn toàn bộ key để họa sĩ chỉ việc đặt file đúng vị trí.

## Background coverage

Hệ thống có asset riêng cho:

- toàn bộ cửa sổ;
- cột giữa, sidebar, right panel và launch bar;
- từng page: Home, Accounts, Instances, Instance Settings, Launcher Settings, Logs và About;
- Mod Manager, Modrinth Browser, Update Dialog và QMessageBox;
- card, hero card, inset panel, group box, tabs, table, log và detail output;
- toàn bộ trạng thái button, input một dòng/nhiều dòng, checkbox, combo box, progress bar, scrollbar, tooltip và badge;
- logo, icon điều hướng, icon hành động và icon trạng thái.

## Fallback behavior

Khi theme hoặc PNG không tải được:

1. Launcher không dừng khởi động.
2. Asset lỗi bị bỏ qua riêng lẻ.
3. Thành phần tương ứng dùng `APP_STYLE` mặc định.
4. Logo và icon quay về text hoặc trạng thái không icon.
5. Theme khác vẫn có thể được chọn trong Launcher Settings.
